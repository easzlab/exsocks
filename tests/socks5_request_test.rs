...
async fn test_parse_unsupported_command_bind() {
    let (mut client, mut server) = common::create_tcp_pair().await;
    // CMD_BIND = 0x02
    let request = vec![SOCKS5_VERSION, CMD_BIND, 0x00, ATYP_IPV4, 127, 0, 0, 1, 0x00, 0x50];
    let handle = tokio::spawn(async move {
        parse_request(&mut server).await
    });
    client.write_all(&request).await.unwrap();
...
async fn test_parse_unsupported_command_udp() {
    let (mut client, mut server) = common::create_tcp_pair().await;
    let request = vec![SOCKS5_VERSION, CMD_UDP_ASSOCIATE, 0x00, ATYP_IPV4, 127, 0, 0, 1, 0x00, 0x50];
    let handle = tokio::spawn(async move {
        parse_request(&mut server).await
    });
    client.write_all(&request).await.unwrap();
...
async fn test_parse_unsupported_atyp() {
    let (mut client, mut server) = common::create_tcp_pair().await;
    // 使用无效的地址类型 0x05
    let request = vec![SOCKS5_VERSION, CMD_CONNECT, 0x00, 0x05, 127, 0, 0, 1, 0x00, 0x50];
    let handle = tokio::spawn(async move {
        parse_request(&mut server).await
    });
    client.write_all(&request).await.unwrap();
